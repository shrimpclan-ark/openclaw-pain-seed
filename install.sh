...
# ── Step 3: 資料目錄 ─────────────────────────────────────
info "Step 3/6: 建立資料目錄..."
mkdir -p "$HOME/.9router"
ok "$HOME/.9router"

# ── Step 4: 9router ───────────────────────────────────────
info "Step 4/6: 部署 9router AI 閘道..."
JWT_SECRET="pain-$(openssl rand -hex 16)"
ADMIN_PASS="pw-$(openssl rand -hex 8)"

docker pull decolua/9router:0.4.71 > /dev/null 2>&1 || true
...
# ── Step 5: 9router API Key 預註冊 ─────────────────────────
info "Step 5/6: 預註冊 API key 到 9router..."
# 這一步至關重要：OpenClaw 從容器內連 9router 需要預先註冊的 API key
# 缺少這步會導致 Dashboard 聊天報 401 (陷阱 #9)
sleep 3  # 等 9router 初始化完成
...
stmt.run(1, \"sk-9router\", \"openclaw\", \"\", 1, new Date().toISOString());
console.log(\"OK\");
"' 2>&1 | grep -q OK && ok "API key sk-9router 已註冊到 9router DB" || warn "API key 註冊失敗（可之後手動執行）"

# ── Step 6: Claude Code ───────────────────────────────────
info "Step 6/6: 設定 Claude Code..."
mkdir -p ~/.claude
...
echo "╔══════════════════════════════════════════════════╗"
echo "║                                                  ║"
echo -e "║     ${GREEN}🎉 PAIN-000 啟動完成 (v4.4)${NC}              ║"
echo "║                                                  ║"
echo -e "║  ${CYAN}🌐 9router${NC}     http://localhost:20128       ║"
...
echo "║                                                  ║"
echo "║  🔧 有問題？                                     ║"
echo "║    bash scripts/lobster-skillet.sh --fix          ║"
echo "║                                                  ║"
echo "╚══════════════════════════════════════════════════╝"
